//Written by Dantong Zhu July/15/24
run("Clear Results");  
close("*");
...
function processImage(imageTitle) {
    ...
    run("Enhance Contrast...", "saturated=0.25 normalize");//added for Round 2 pics
    setAutoThreshold("Moments dark");
    setOption("BlackBackground", true);
    run("Convert to Mask", "method=Moments background=Dark calculate black");
    run("Watershed", "stack");
    run("Analyze Particles...", "size=0.8-10 circularity=0.20-1.00 show=Nothing exclude summarize stack");
}
...
wait(5);
run("Set Scale...", "distance=2448 known=434.64 unit=micron global");// this is unique to our imaging system
selectImage("Result");
run("Analyze Particles...", "size=0.05-0.2 circularity=0.2-1.00 show=Nothing summarize stack"); //Count particles smaller than 0.2 500nm-
run("Analyze Particles...", "size=0.2-0.8 circularity=0.2-1.00 show=Nothing summarize stack");   //Count particles between 0.2-0.8 500nm-1um
run("Analyze Particles...", "size=0.8-5 circularity=0.3-1.00 show=Nothing summarize stack");   //Count particles larger than 0.8 1um+ 
